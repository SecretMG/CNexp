...
class list {
    static int[] crc_list = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084,
...
public class CRC {
    ...
    public static boolean crc_check(byte[] bin_pack) {
//        byte[] wocbin = new byte[bin_pack.length - 2];
//        System.arraycopy(bin_pack, 0, wocbin, 0, bin_pack.length - 2);
//        System.out.println(convertBytesToHex(wocbin));
//        MyPacket pkt = null;
//        try {
//            pkt = (MyPacket) serializer.toObject(wocbin);
//        } catch (IOException e) {
//            e.printStackTrace();
//        } catch (ClassNotFoundException e) {
//            e.printStackTrace();
//        }
//        System.out.println(pkt.sequence_num+pkt.data);
        int crc = 0x0000;
        for (int i = 0; i < bin_pack.length; i++)
            crc = list.crc_list[((crc >> 8) ^ bin_pack[i]) & 0xFF] ^ (crc << 8) % (0xFFFF + 1);
        if (crc != 0) {
//            System.out.println(crc);
    ...
    public static int __get_check_sum(byte[] pre_bin_pack) {
        int crc = 0x0000;
        for (int i =  0; i < pre_bin_pack.length; i++)
            crc = list.crc_list[((crc >> 8) ^ pre_bin_pack[i]) & 0xFF] ^ (crc << 8) % (0xFFFF + 1);
...
//        System.out.println(strCrc);
    ...
    public static byte[] __init__(byte[] buf) {
        int checkCode = 0;
        checkCode = __get_check_sum(buf);
...
//        byte[] data= {(byte) 0xAA, 0x0C, 0x01, 0x00, 0x01, 0x00, 0x00, 0x04, 0x05, 0x7b, 0x05, 0x01};
        MyPacket udp_pkt = new MyPacket(1,(byte)-1,"wow");
        byte[] data = serializer.toBytes(udp_pkt);
        System.out.println(convertBytesToHex(data));
        byte[] crcData = __init__(data);

        System.out.println(convertBytesToHex(crcData));

        if(crc_check(crcData)){
